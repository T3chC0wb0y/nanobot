...
class AgentLoop:
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        if turn_continuation.internal_continuation_inbound(ctx.msg.metadata):
            ctx.user_persisted_early = False
        else:
            ctx.user_persisted_early = self._persist_user_message_early(
                ctx.msg, ctx.session
            )
    ...
    async def _state_save(self, ctx: TurnContext) -> str:
        ...
        skip_current_user_message = (
            ctx.user_persisted_early
            or turn_continuation.internal_continuation_inbound(ctx.msg.metadata)
        )
        ctx.save_skip = 2 + len(ctx.history) + (1 if skip_current_user_message else 0)
        ...
        if ctx.user_persisted_early and save_skip >= len(ctx.all_messages) and ctx.all_messages:
            # Some tests and custom loop stubs return only a partial transcript.
            # If there is a final assistant message, preserve it; never lower
            # save_skip far enough to re-save an already persisted user message.
            last_message = ctx.all_messages[-1]
